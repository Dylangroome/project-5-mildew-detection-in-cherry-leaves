...
def page_leaf_mildew_detector_body():
    st.info(
        f"* The client is interested in telling whether a given leaf contains powdery mildew "
        f"or not."
    )

    st.write(
        f"* You can download a set of healthy and powdery mildew cherry leave images for live prediction. "
        f"You can download the images from [here](https://www.kaggle.com/datasets/codeinstitute/cherry-leaves)."
    )

    st.write("---")

    images_buffer = st.file_uploader("Upload cherry leaf images. You can select multiple.",
